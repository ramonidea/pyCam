...
def callback(data):
    ...
    latency += int(time.time()) - timestamp.secs
    if (int(round(time.time() * 1000)) - lasttime > 5000):
        ...
        count = 0
    if(len(fps) > 100 ):
        with open("comfps.json","wb") as outfile:
            json.dump(fps,outfile)
        with open("comlat.json","wb") as outfile:
...
def callback1(data):
    ...
    timestamp = data.header.stamp
    latency1 += int(time.time()) - timestamp.secs
    if (int(round(time.time() * 1000)) - lasttime1 > 5000):
        ...
        count1 = 0
    if(len(fps1) > 100 ):
        with open("comfps1.json","wb") as outfile:
            json.dump(fps1,outfile)
        with open("comlat1.json","wb") as outfile:
